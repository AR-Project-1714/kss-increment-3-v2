...
<style>
        ...
        @media (max-width: 768px) {
            ...
            /* Sticky island should stay compact â€” center override */
            .content-header.is-sticky {
        ...
        @media (max-width: 480px) {
            ...
            .tab-sections { min-width: 80px !important; font-size: 11px !important; padding: 8px !important; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function applyMasterDatalists(root = document) {
        root.querySelectorAll('input[type="text"], input:not([type])').forEach(input => {
            ...
            if (/(employee_shift_logs|relief_logs|overtime_logs|op7_logs|replacement_logs|other_activity_logs|driver|operator|foreman|tally|stevedoring|petugas)/i.test(name)) {
                input.setAttribute('list', 'master-employee-list');
            }

            if (/(truck_name|truck_number)/i.test(name)) {
                input.setAttribute('list', 'master-truck-list');
            }

            if (/(forklift|unit_logs\[[^\]]+\]\[item_name\])/i.test(name)) {
    ...
    function applyShiftTimesToRow(row) {
        ...
        if (isAbsentDescription(description)) {
            timeInInput.value = '';
            timeOutInput.value = '';
            return;
        }
    ...
    function clearRow(row) {
        ...
        row.querySelectorAll('input, textarea, select').forEach(input => {
            ...
            input.value = '';
            if (!keepReadonly) input.removeAttribute('readonly');
        });
    }
...
@php
    try {
        $documentYear = ($report->report_date ?: $report->created_at)
            ? \Carbon\Carbon::parse($report->report_date ?: $report->created_at)->format('Y')
            : now()->format('Y');
    } catch (\Throwable) {
...
@section('content')
    <!-- MAIN APP WRAPPER -->
    <form id="mainReportForm" action="{{ route('report-ops.update', $report) }}" method="POST" class="content d-flex flex-column align-items-start align-self-stretch gap-30 p-content">
        @csrf
        @method('PUT')
        ...
        <div class="content-header">
            <div class="title-header">
                <span class="text-header fw-600 fsize-20">Edit Laporan Shift Harian</span>
                <span class="note fw-300 fsize-12 text-secondary">ID: {{ $documentId }}</span>
            </div>
            <button type="button" id="btnSaveDraft" class="btn-new d-flex justify-content-center align-items-center gap-10">
                ...
                <span class="btn-text fw-500">Simpan Pembaruan</span>
            </button>
        </div>
            ...
            <div class="pop-up-content d-flex flex-column gap-15">
                <div class="pop-up detail d-flex align-items-center">
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <span class="fsize-10 text-secondary">ID: {{ $documentId }}</span>
                    </div>
                </div>
                <p class="fsize-12 text-muted m-0">Setelah dikirim, laporan ini akan diteruskan ke sistem pusat untuk divalidasi. Pastikan tidak ada data yang terlewat.</p>
            </div>
