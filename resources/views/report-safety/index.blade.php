...
<style>
    /* Lebar kolom tabel riwayat diatur oleh kontrak bersama
       ...
       Di sini hanya badge khas modul K3 dan warna tombol cetak. */
    ...
    <div class="content d-flex flex-column align-items-start align-self-stretch gap-30 p-content">
        <div class="content-header d-flex justify-content-between align-items-center align-content-center align-self-stretch flex-wrap p-20" style="row-gap:10px;border-radius:16px">
            <div class="title-header d-flex flex-column align-items-start gap-2 flexible">
                <span class="text-header fw-600 fsize-20 align-self-stretch">Laporan Keselamatan (K3)</span>
                <span class="note fw-300 fsize-12 text-secondary align-self-stretch">Kelola laporan harian keselamatan & kesehatan kerja: draft, riwayat, dan buat laporan baru.</span>
            </div>
            <a href="{{ route('safety.create') }}" class="btn-new d-flex justify-content-center align-items-center gap-10 br-12" style="cursor:pointer;text-decoration:none">
                <div class="icon-new"><i class="fi fi-rr-add"></i></div>
                <span class="white-pure fsize-14 fw-500">Buat Laporan K3</span>
            </a>
        </div>
                ...
                <div class="reminder d-flex align-items-center gap-10 flexible" style="min-width:300px;">
                    ...
                    <div class="text-reminder d-flex flex-column align-items-start flexible" style="gap:2px;">
                        ...
                        <span class="fsize-9 fw-400 align-self-stretch">
                            Anda memiliki <span class="fw-600">{{ $draftReports->count() }} draft</span> laporan harian K3 yang belum diselesaikan.
                        </span>
                    </div>
                </div>
                        ...
                        <div class="draft-report">
                            <div class="draft-detail">
                                <div class="draft-title d-flex flex-column align-items-start align-self-stretch">
                                    <span class="title fsize-16 fw-600 text-main align-self-stretch">Draft Laporan Harian K3</span>
                                ...
                                @include('partials.draft-expiry', [
                                    'report' => $report,
                                    'ttlDays' => \App\Models\SafetyReport::DRAFT_TTL_DAYS,
                                    'extendUrl' => route('safety.extend-draft', $report),
                                ])
                            </div>
                ...
                @empty
                    <div class="empty-state d-flex flex-column align-items-center align-self-stretch gap-10">
                        ...
                        <div class="empty-text d-flex flex-column align-items-center align-self-stretch" style="gap:5px;">
                            ...
                            <span class="text-center fsize-12 text-secondary">Belum ada draft laporan K3 yang perlu diselesaikan.</span>
                        </div>
                    </div>
                @endforelse
            </div>
                        ...
                        @forelse ($historyReports as $report)
                            ...
                            <tr class="tbody d-flex align-items-center align-self-stretch" role="row">
                                ...
                                <td class="col-doc cell-stack" role="cell">
                                    <span class="cell-title">Laporan Harian K3</span>
                                ...
                                <td class="aksi col-action" role="cell">
                                    <a href="{{ route('safety.show', $report) }}" class="btn see">
                                        <span><i class="fi fi-rr-eye"></i></span><span>Lihat</span>
                                    </a>
                                    <a href="{{ route('safety.pdf', $report) }}" class="btn export-pdf" target="_blank" rel="noopener">
                                        <span><i class="fi fi-rr-file-pdf"></i></span><span>PDF</span>
                                    </a>
                                    @if ($report->status === SafetyStatus::Submitted)
                                        <a href="{{ route('safety.edit', $report) }}" class="btn edit">
                                            <span><i class="fi fi-rr-pencil"></i></span><span>Edit</span>
                                        </a>
                                    @endif
                                    <a href="{{ route('safety.show', $report) }}?print=1" class="btn print-report" target="_blank" rel="noopener" title="Cetak laporan" aria-label="Cetak laporan">
                            ...
                            <tr class="row-full">
                                <td colspan="7" class="border-0 p-0">
                                    <div class="empty-state d-flex flex-column align-items-center justify-content-center align-self-stretch gap-10 w-100">
                                        ...
                                        <a href="{{ route('safety.create') }}" class="btn new-report">
        ...
        <div class="modal-overlay" id="continue-draft-modal-{{ $report->id }}">
            <div class="pop-up signed d-flex flex-column gap-20">
                ...
                <div class="pop-up-content d-flex flex-column gap-15">
                    ...
                    <p class="fsize-12 text-muted m-0">Draft laporan K3 akan dibuka kembali dengan data terakhir yang tersimpan agar bisa dilanjutkan.</p>
                </div>
                <div class="pop-up footer d-flex justify-content-end gap-10 flex-wrap">
                    ...
                    <a href="{{ route('safety.edit', $report) }}" class="btn edit-confirm">
        ...
        <div class="modal-overlay" id="delete-draft-modal-{{ $report->id }}">
            <div class="pop-up signed d-flex flex-column gap-20">
                ...
                <div class="pop-up detail danger d-flex align-items-center">
                    ...
                    <div>
                        <div class="fw-600 fsize-13 text-main">Draft Laporan Harian K3</div>
                ...
                <div class="pop-up footer d-flex justify-content-end gap-10">
                    <button type="button" class="btn cancel" data-close-modal>Batal</button>
                    <form action="{{ route('safety.destroy', $report) }}" method="POST" class="m-0">
                        @csrf @method('DELETE')
        ...
        <div class="modal-overlay" id="delete-all-drafts-modal">
            <div class="pop-up signed d-flex flex-column gap-20">
                ...
                <div class="pop-up detail danger d-flex align-items-center">
                    <span class="icon-document danger"><i class="fi fi-rr-trash"></i></span>
                    <div>
                        <div class="fw-600 fsize-13 text-main">{{ $draftReports->count() }} draft laporan harian K3</div>
                ...
                <div class="pop-up footer d-flex justify-content-end gap-10">
                    <button type="button" class="btn cancel" data-close-modal>Batal</button>
                    <form action="{{ route('safety.drafts.destroy-all') }}" method="POST" class="m-0">
                        @csrf @method('DELETE')
